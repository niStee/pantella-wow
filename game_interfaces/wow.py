...
if (
    os.path.basename(os.path.dirname(os.path.abspath(__file__))) == "game_interfaces"
    and addon_root not in sys.path
):
...
class WoWGameInterface(BaseGameInterface):
    ...
    def _find_wow_window(self):
        ...
        def enum_child(hwnd, extra):
            if (
                ...
                and win32gui.GetWindowText(hwnd) == "MantellaWoW_State"
            ):
    ...
    def _read_editbox_text(self):
        ...
        try:
            ...
            win32gui.SendMessage(self.editbox_hwnd, win32con.WM_GETTEXT, max_length + 1, buffer)
            raw = bytes(buffer)
    ...
    def _score_event(self, event_type, data):
        ...
        # Event scoring keys reference GOSSIP_SHOW / QUEST_ACCEPTED / QUEST_COMPLETE
        # https://wowpedia.fandom.com/wiki/GOSSIP_SHOW
        # https://wowpedia.fandom.com/wiki/QUEST_ACCEPTED
    ...
    def _generate_reaction(self, event, pet):
        ...
        if etype == "gossip_show":  # https://wowpedia.fandom.com/wiki/GOSSIP_SHOW
        ...
        if etype == "quest_accepted":  # https://wowpedia.fandom.com/wiki/QUEST_ACCEPTED
            return "[SYSTEM: Your master has accepted a new quest. React with encouragement or concern based on your personality.]"
        if etype == "quest_complete":  # https://wowpedia.fandom.com/wiki/QUEST_COMPLETE
    ...
    def get_system_prompt(self):
        ...
        # Pet health is derived from UnitHealth()/UnitHealthPercent(); death state from UnitIsDeadOrGhost()
        ...
        if hostiles:
            env_lines.append(
                f"Hostile units detected: {', '.join(hostiles)}. They are flagged as enemies."
            )
        if npcs:
    ...
    def check_radiant_triggers(self):
        ...
        if current_health < 25 and self.last_pet_health >= 25:
            ...
            triggers.append(
                {
                    "text": f"{name} is critically wounded and near death! Heal me, master!",
                    "priority": "urgent",
                    "color": "red",
                }
            )
            ...
            triggers.append(
                {
                    "text": "I am injured, master. The battle is taking its toll. I need healing.",
                    "priority": "high",
                    "color": "orange",
                }
            )
        ...
        if pet.get("is_dead") and not self.pet_was_dead:
            triggers.append(
                {
                    "text": "I have fallen... my spirit fades. Do not forget me, master...",
                    "priority": "urgent",
                    "color": "red",
                }
            )
        ...
        for timer in state.get("dbm_timers", []):
            ...
            if time_remaining < 5 and self.last_dbm_timers.get(timer_id, 999) >= 5:
                ...
                triggers.append(
                    {
                        "text": f"Brace yourself! {ability} is incoming in mere seconds!",
                        "priority": "urgent",
                        "color": "red",
                    }
                )
    ...
    def get_current_context_string(self):
        ...
        if "player_name" in state:
            ctx_parts.append(
                f"Player: {_sanitise(state['player_name'], max_len=48)} (Level {state.get('player_level', '?')})"
            )
        if "zone" in state:
        ...
        if "active_quests" in state and isinstance(state["active_quests"], list):
            ...
            ctx_parts.append(
                f"Active Quests: {', '.join(_sanitise(q.get('name', '?'), max_len=64) for q in quests)}"
            )
        if "dbm_timers" in state and isinstance(state["dbm_timers"], list):
            timers = state["dbm_timers"][:3]
            ctx_parts.append(
                f"Boss Timers: {', '.join(_sanitise(t.get('message', '?'), max_len=64) for t in timers)}"
            )

...
def _state(
    health=100, is_dead=False, dbm_timers=None, recent_events=None, chattyness=3, token="PET"
):
...
class TestHealthThresholdTriggers(unittest.TestCase):
    ...
    def test_critical_trigger_color_is_red(self):
        ...
        critical = [
            t
            for t in triggers
            if "critically wounded" in t["text"].lower() or "near death" in t["text"].lower()
        ]
    ...
    def test_injured_trigger_fires_below_50(self):
        ...
        texts = [t["text"] for t in triggers]
        self.assertTrue(
            any("injured" in t.lower() for t in texts), f"Expected injured trigger, got: {texts}"
        )
    ...
    def test_healthy_pet_produces_no_health_trigger(self):
        ...
        health_triggers = [
            t
            for t in triggers
            if "injured" in t["text"].lower() or "near death" in t["text"].lower()
        ]
...
class TestDeathTriggers(unittest.TestCase):
    ...
    def test_death_trigger_color_is_red(self):
        ...
        triggers = iface.check_radiant_triggers()
        death = [
            t for t in triggers if "fallen" in t["text"].lower() or "spirit" in t["text"].lower()
        ]
...
class TestDBMTimerTriggers(unittest.TestCase):
    ...
    def test_dbm_trigger_color_is_red(self):
        ...
        triggers = iface.check_radiant_triggers()
        dbm_triggers = [
            t for t in triggers if "brace" in t["text"].lower() or "slam" in t["text"].lower()
        ]

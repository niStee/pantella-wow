...
class TestPromptBuilding:
    ...
    def test_voidwalker_personality(self, interface):
        ...
        assert "sardonic" in prompt.lower()
    ...
    def test_low_health_status(self, interface):
        ...
        prompt = interface.get_system_prompt()
        assert "critically wounded" in prompt.lower() or "agony" in prompt.lower()
    ...
    def test_dead_pet_status(self, interface):
        ...
        prompt = interface.get_system_prompt()
        assert "slain" in prompt.lower() or "spirit" in prompt.lower()
...
class TestRadiantTriggers:
    ...
    def test_critical_health_trigger(self, interface):
        ...
        assert any("critically wounded" in t["text"] or "near death" in t["text"] for t in triggers)
    ...
    def test_pet_death_trigger(self, interface):
        ...
        triggers = interface.check_radiant_triggers()
        assert any(t["priority"] == "urgent" for t in triggers)
        assert any("fallen" in t["text"] or "spirit fades" in t["text"] for t in triggers)
            ...
            "recent_events": [
                {
                    ...
                    "type": "zone",
                    "data": "Stormwind",
                },  # COMPANION zone score is 8 (>=7), should trigger
                {
                    "id": 2,
                    "type": "combat",
                    "data": "Enter Combat",
                },  # COMPANION combat score is 2 (<7), ignored
                {
                    "id": 3,
                    "type": "chat",
                    "data": "Thrall: hello",
                },  # COMPANION chat score is 9 (>=7), should trigger
            ],
        }
...
class TestStateParsing:
    ...
    def interface(self, mock_conversation_manager):
        with (
            patch("game_interfaces.wow.WIN32_AVAILABLE", False),
            patch("game_interfaces.wow.WATCHDOG_AVAILABLE", False),
            patch("game_interfaces.wow.WINSOUND_AVAILABLE", True),
        ):
            iface = WoWGameInterface(

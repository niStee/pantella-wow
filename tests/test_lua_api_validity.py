...
def lua_docs_source():
    assert LUA_DOCS.exists(), (
        "docs/wow_lua_api_used.md not found. "
...
class TestRegisteredEventsCoverage:
    ...
    def test_no_undocumented_events_registered(self, registered_events):
        ...
        assert not undocumented, (
            "Events registered in MantellaWoW.lua but not documented:\n"
...
LUA_APIS = {
    ...
    'UnitHealth("pet")': "https://wowpedia.fandom.com/wiki/UnitHealth",
    'UnitHealthMax("pet")': "https://wowpedia.fandom.com/wiki/UnitHealthMax",
    "C_UnitAuras.GetAuraDataByIndex(": "https://wowpedia.fandom.com/wiki/C_UnitAuras.GetAuraDataByIndex",
    # Zone & World
...
class TestDeprecationWatch:
    ...
    These tests now assert that the migration has been applied.
    """

    def test_unit_aura_migrated(self, lua_source):
        """
        ...
        """
        assert "C_UnitAuras.GetAuraDataByIndex(" in lua_source
        assert "UnitAura(" not in lua_source, (
            "UnitAura() was deprecated in patch 10.1. Use C_UnitAuras.GetAuraDataByIndex() instead."
        )

    def test_health_percent_migrated(self, lua_source):
        """
        UnitHealthPercent with CurveConstants is no longer relied upon.
        Use UnitHealth() / UnitHealthMax() to compute the percentage directly.
        Wowpedia: https://wowpedia.fandom.com/wiki/UnitHealth
        """
        assert 'UnitHealth("pet")' in lua_source
        assert 'UnitHealthMax("pet")' in lua_source
        assert "CurveConstants" not in lua_source, (
            "CurveConstants.ScaleTo100 is unreliable; compute percentage from "
            "UnitHealth / UnitHealthMax instead."
        )
        assert "UnitHealthPercent(" not in lua_source, (
            "UnitHealthPercent() is no longer used; compute percentage from "
            "UnitHealth / UnitHealthMax instead."
        )
